...
(function ($) {
  ...
  var MirrorlyAdmin = {
    ...
    initProductSelector: function () {
      $('#select-products').on('click', function (e) {
        e.preventDefault();
    ...
    initImagePreview: function () {
      $('#_mirrorly_image').on('change', function () {
    ...
    initStylePreview: function () {
      ...
      $('.mirrorly-color-picker').on('change', function () {
        MirrorlyAdmin.updatePreview();
      });

      // Update preview when styling options change
      $('select[name*="widget_styling"], input[name*="widget_styling"]').on('change input', function () {
        MirrorlyAdmin.updatePreview();
      });
    },
    ...
    initProductMessages: function () {
      $(document).on('click', '.save-product-message', function (e) {
        e.preventDefault();
    ...
    updatePreview: function () {
      ...
      $('.mirrorly-color-picker').each(function () {
        var $this = $(this);
      ...
      $('select[name*="widget_styling"], input[name*="widget_styling"]').each(function () {
        var $this = $(this);
        var name = $this.attr('name');
        if (name) {
    ...
    openProductSelector: function () {
      ...
      // Modal events
      $('.mirrorly-modal-close, #mirrorly-modal-cancel').on('click', function () {
        modal.remove();
      });

      $('.mirrorly-modal-overlay').on('click', function (e) {
        if (e.target === this) {
          modal.remove();
        }
      });

      $('#mirrorly-add-selected').on('click', function () {
        MirrorlyAdmin.addSelectedProducts();
        modal.remove();
      });

      // Search functionality
      $('#mirrorly-product-search').on('input', function () {
        var query = $(this).val();
    ...
    addSelectedProducts: function () {
      ...
      $('.mirrorly-product-item input:checked').each(function () {
