  ...
  var MirrorlyFrontend = {
    ...
    preloadResources: function () {
      ...
      if (mirrorly_frontend.preload_images) {
        mirrorly_frontend.preload_images.forEach(src => {
          const img = new Image();
          img.src = src;
        });
      }
    ...
    initButtons: function () {
      ...
      // Change image button
      $('#mirrorly-change-image').on('click', function () {
        self.resetToUpload();
      });

      // Generate button
      $('#mirrorly-generate-btn').on('click', function () {
        self.generateImage();
      });

      // Download button
      $('#mirrorly-download-btn').on('click', function () {
        self.downloadImage();
      });

      // Share button
      $('#mirrorly-share-btn').on('click', function () {
        self.shareImage();
      });

      // Try again button
      $('#mirrorly-try-again-btn').on('click', function () {
        self.showSection('preview');
      });

      // Retry button
      $('#mirrorly-retry-btn').on('click', function () {
        self.generateImage();
      });
    },
    ...
    showShareOptions: function () {
      ...
      modal.find('.mirrorly-share-close').on('click', function () {
        modal.remove();
      });
