...
if ( $license->is_pro() ) {
	...
	if ( $widget_styling['animation'] !== 'none' ) {
		$widget_classes[] = 'animation-' . $widget_styling['animation'];
	}
}
?>

<div id="mirrorly-widget" class="<?php echo implode( ' ', $widget_classes ); ?>" data-product-id="<?php echo esc_attr( $product->get_id() ); ?>"
	<div class="mirrorly-header">
		...
		<?php if ( ! $license_status['can_generate'] ) : ?>
			<div class="mirrorly-limit-notice">
				<?php if ( $license_status['remaining_generations'] === 0 ) : ?>
					<p class="mirrorly-error">
						<?php _e( 'Has alcanzado el límite de generaciones para este mes.', 'mirrorly' ); ?>
						<?php if ( $license->is_free() ) : ?>
							<a href="<?php echo admin_url( 'admin.php?page=mirrorly-settings' ); ?>" class="mirrorly-upgrade-link">
		...
		<?php else : ?>
			<div class="mirrorly-usage-info">
				<?php if ( $license_status['remaining_generations'] !== 'unlimited' ) : ?>
					<p class="mirrorly-remaining">
						<?php printf( __( 'Generaciones restantes: %s', 'mirrorly' ), '<strong>' . $license_status['remaining_generations'] . '</strong>' ); ?>
					</p>
				<?php endif; ?>
			</div>
		<?php endif; ?>
	...
	<?php if ( $license_status['can_generate'] ) : ?>
			...
			<div class="mirrorly-upload-area" id="mirrorly-upload-area">
				<div class="mirrorly-upload-content">
					...
					<p class="mirrorly-upload-text">
						<?php _e( 'Arrastra tu foto aquí o haz clic para seleccionar', 'mirrorly' ); ?>
					</p>
					<p class="mirrorly-upload-hint">
						<?php
						$max_size = $license->is_pro() ? '5MB' : '2MB';
						printf( __( 'JPG, PNG o GIF (máx. %s)', 'mirrorly' ), $max_size );
						?>
					</p>
				</div>
		...
		<div class="mirrorly-generation-section" id="mirrorly-generation-section" style="display: none;">
			<div class="mirrorly-loading" id="mirrorly-loading">
				...
				<p class="mirrorly-loading-text"><?php _e( 'Generando tu imagen personalizada...', 'mirrorly' ); ?></p>
				<p class="mirrorly-loading-hint"><?php _e( 'Esto puede tomar unos segundos', 'mirrorly' ); ?></p>
			...
			<div class="mirrorly-result" id="mirrorly-result" style="display: none;">
				<div class="mirrorly-result-container">
					<img id="mirrorly-result-image" src="" alt="<?php _e( 'Resultado generado', 'mirrorly' ); ?>">
			...
			<div class="mirrorly-error" id="mirrorly-error" style="display: none;">
				<div class="mirrorly-error-content">
					...
					<button type="button" id="mirrorly-retry-btn" class="mirrorly-btn mirrorly-btn-primary">
						<?php _e( 'Intentar de nuevo', 'mirrorly' ); ?>
					</button>
				</div>
			</div>
		</div>
	...
	<?php if ( $license->is_free() ) : ?>
		<div class="mirrorly-footer">
			<p class="mirrorly-upgrade-notice">
				<?php _e( '¿Quieres más generaciones y funciones avanzadas?', 'mirrorly' ); ?>
				<a href="<?php echo admin_url( 'admin.php?page=mirrorly-settings' ); ?>" class="mirrorly-upgrade-link">
					<?php _e( 'Actualizar a PRO', 'mirrorly' ); ?>
				</a>
			</p>
